...
PROCEDURE pi-get:
    ...
    IF oQueryParams:HAS("shopifyId") THEN
        ASSIGN cShopifyId = oQueryParams:GetJsonArray("shopifyId"):getCharacter(1) NO-ERROR.

         


    FIND FIRST es-spf-cliente WHERE es-spf-cliente.shopify-id = cShopifyId NO-LOCK NO-ERROR.
    IF NOT AVAIL es-spf-cliente THEN
    DO:
