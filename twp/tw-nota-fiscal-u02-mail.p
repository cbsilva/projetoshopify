...
define temp-table tt-erros-mail
    field cod-erro  as integer
    field desc-erro as character format "x(256)"
    field desc-arq  as character.


def var ccodestabel as char.
def var cserie      as char.
def var cnrnotafis  as char.
def var cArquivoxml as char.

DEFINE VARIABLE c-cod-dir-histor-xml-nfse  AS CHARACTER   NO-UNDO.
DEFINE VARIABLE cChaveAcesso               AS CHARACTER   NO-UNDO.
DEFINE VARIABLE cdirxml                AS CHARACTER   NO-UNDO.
...
//    RUN pi-imprime-nota (output c-arq-email).
    ...
    RUN repositionRecord IN h-bodi135na (INPUT rowid(b-nota)).
    ...
    RUN goToKey          IN h-bodi520   (INPUT cCodEstabel).
                                       
    RUN getCharField     IN h-bodi520   (INPUT  "cod-dir-histor-xml-nfse":U,
                                         OUTPUT c-cod-dir-histor-xml-nfse).
    

    ASSIGN cdirxml = TRIM(c-cod-dir-histor-xml-nfse) + '\' 
                           + (ccodestabel) 
                           + (cserie) 
                           + cnrnotafis.


    RUN leituraArquivos (OUTPUT cArquivoxml).
    ...
    assign tt-envio2.versao-integracao = 1
           ...
           tt-envio2.arq-anexo   = cArquivoXml
...
PROCEDURE leituraArquivos.

    DEF OUTPUT PARAMETER cArquivo AS CHAR.
    ...
    */
    ...
    DO i = 1 TO NUM-ENTRIES(list-char):
    
        //MESSAGE cdirxml + "\" + ENTRY(i,list-char) VIEW-AS ALERT-BOX.
        cArquivo = cdirxml + "\" + ENTRY(i,list-char).
        
        IF SEARCH(cArquivo) <> ? THEN LEAVE.
