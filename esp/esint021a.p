       ...
       FOR FIRST emitente WHERE emitente.cgc = ttCustomer.CNPJ NO-LOCK:
          ...
          BUFFER-COPY emitente TO tt-emitente
             ASSIGN tt-emitente.nome-emit        = ttCustomer.RazaoSocial
                    ...
                    tt-emitente.Ind-cre-cli      = 2 /*-- Autom†tico --*/  
          ...
          IF tt-emitente.identific = 2 THEN
              ASSIGN tt-emitente.Nat-operacao  = es-api-param-cliente.nat-operacao 
...
PROCEDURE piCriaCliente:
   ...
   DO TRANS 
      ...
      if (fnNatureza(ttCustomer.cnpj) = 1) THEN
         ASSIGN iGrupoCli = es-api-param-cliente.cod-gr-cli-fisica.
      else
      ASSIGN iGrupoCli = es-api-param-cliente.cod-gr-cli.
      ...
      ASSIGN tt-emitente.cod-emitente                = iCodEmitente
             ...
             tt-emitente.cod-canal-venda             = es-api-param-cliente.cod-canal-venda.
       ...
       IF  param-global.log-2 = YES THEN DO:                     
           ...
           RUN cdp/cd1608.p persistent set h-cd1608  
            ...
            DO:
                ...
                IF NOT CAN-FIND(FIRST es-spf-cliente WHERE es-spf-cliente.cod-emitente = tt-emitente.cod-emitente) THEN
                DO: 
                    PUT UNFORMATTED "GERANDO TABELA AUXILIAR es-spf-cliente" SKIP.
                    CREATE es-spf-cliente.                                                                
                    ASSIGN es-spf-cliente.cod-emitente = tt-emitente.cod-emitente                            
                           es-spf-cliente.shopify-id   = ttCustomer.shopifyId.                            
                END.                                                                                    
            END.
       END.                                                      
   END.
...
PROCEDURE piValidaEmitente:
   ...
   IF ttCustomer.ShopifyID <> "" AND CAN-FIND(FIRST es-spf-cliente WHERE es-spf-cliente.shopify-id = ttCustomer.ShopifyID) AND NOT lUpdate THEN
   DO: 
