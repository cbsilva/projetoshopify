 ...
 *  OBJETIVO        API Integra‡äes JSON  - baseado no esspf001 (hist¢rico abaixo)
 *  AUTOR           TOTVS - LASF
 *  DATA            12/2020
 *------------------------------------------------------------------------------
 */
...
LOG-MANAGER:WRITE-MESSAGE("INICIO DO PROCESSO - ESSPF100IRP - V.1.1 - MACMILLAN - Importa‡Æo").
        ...
        DO:
            ...
            FOR EACH  bf-es-api-import-spf 
                ...
                LOG-MANAGER:WRITE-MESSAGE("INI - Processando CD-TIPO-TRANS:" + STRING(bf-es-api-import-spf.cd-tipo-integr) 
                                + " Situa‡Æo:" + STRING(bf-es-api-import-spf.ind-situacao)
...
PROCEDURE pi-processa-transacao:
    ...
    //SenÆo tiver registro e nÆo estiver locado sai do bloco
...
/*                 OR  (RETURN-VALUE = "NOK").                                                                     */
        ...
        DO TRANSACTION:
            ...
            THEN DO:
                ASSIGN  es-api-import-spf.data-fim      = NOW
                        ...
                        c-erro                      = "Houve erro ao processar transa‡Æo. " + c-erro.
                    .
            END.
            ELSE IF vLogEfetivacaoNOK /* Erro Travamento de Registro */
            THEN DO:
                ASSIGN  es-api-import-spf.data-fim      = NOW
                        es-api-import-spf.ind-situacao  = 3 // Houve erro de execu‡Æo / reprocesssar
                        es-api-import-spf.chave         = IF c-chave = "" THEN es-api-import-spf.chave ELSE c-chave
                        c-erro                      = "Ocorreu erro progress que impediu a conclusÆo da transa‡Æo. " + c-erro.
                IF ERROR-STATUS:ERROR THEN
                DO:
                    DO iErro = 1 TO ERROR-STATUS:NUM-MESSAGES:
    ...
    // Tratametno erro t‚cnico/LOCKS
    IF INDEX(c-erro, "#ERROTEC") > 0
    THEN DO:
        ASSIGN  es-api-import-spf.data-fim      = NOW
                es-api-import-spf.ind-situacao  = 3 // Erro T‚cnico - Reprocessar
                es-api-import-spf.cod-status    = 0 // NÆo Processado / Na Fila
                es-api-import-spf.chave         = IF c-chave = "" THEN es-api-import-spf.chave ELSE c-chave
                c-erro                      = "Houve erro t‚cnico ao processar transa‡Æo. " + c-erro.
            .
    END.
