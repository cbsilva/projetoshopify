...
DO:
    RUN piErro("Par�metro do cliente n�o cadastrado no ESSPF016").
...
PROCEDURE piAlteraCliente:
    ...
    DO:
       ...
       PUT "OCORREU ERRO NA VALIDA��O DO CLIENTE" SKIP.
   ...
   DO:
       FOR FIRST emitente WHERE emitente.cgc = ttCustomer.CNPJ NO-LOCK:
          ...
          DO:
             ...
             IF RETURN-VALUE = "NOK" THEN 
             DO:
                 PUT UNFORMATTED "OCORREU ERRO NA GERA��O DO EMITENTE NO EMS5" SKIP.
                //RUN piEnviaNotificacaoUsuario("suporte@macmillan.com.br", "cleberson.silva@4make.com.br", "Erro").  
...
PROCEDURE piCriaCliente:
   ...
   DO TRANS 
      ...
      IF CAN-FIND(FIRST tt-bo-erro NO-LOCK) THEN 
      DO:
          PUT UNFORMATTED "OCORREU ERRO NA GERA��O DO EMITENTE" SKIP.
...
PROCEDURE piValidaEmitente:
   ...
   DO:
      RUN piErro("ERRO: Unidade de federa��o n�o cadastrada.").
      RETURN "NOK":U.
   END.
...
PROCEDURE piEnviaNotificacaoUsuario:
   ...
   ASSIGN cAssunto  = "Integra��o de Clientes SHOPIFY"
          cDestino  = pDestino.
   ...
   ELSE
   DO:
      ASSIGN cMensagem = SUBSTITUTE("Prezado(s), Informo que foram gerados, erros ao tentar integrar o cliente &1 - CPF/CNPJ &1, verifique o monitor de integra��o.", ttCustomer.RazaoSocial, ttCustomer.cnpj).
   END.
   ...
   DO:
      ...
      RUN piErro("Houver erro ao enviar notifica��es").
      FOR EACH tt-erros:
