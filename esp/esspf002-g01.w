...
*******************************************************************************/
{include/i-prgvrs.i G01ESCD0002 12.1.10.001}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i G01ESCD0002 MUT}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cd-sistema bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cd-sistema 
...
DEFINE FRAME d-vapara
     i_cd-sistema AT ROW 1.33 COL 22.43 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 2.88 COL 2.14
     bt-cancela AT ROW 2.88 COL 13
     bt-ajuda AT ROW 2.88 COL 38.72
     rt-button AT ROW 2.63 COL 1
     SPACE(0.71) SKIP(0.36)
...
ON GO OF FRAME d-vapara /* V  Para Canal */
DO:
FIND es-api-app-spf NO-LOCK
    WHERE es-api-app-spf.cd-sistema EQ INPUT FRAME {&FRAME-NAME} i_cd-sistema NO-ERROR.

IF NOT AVAILABLE es-api-app-spf THEN DO:
    {utp/ut-table.i mgesp es-api-app-spf 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-api-app-spf).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Canal */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_cd-sistema 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cd-sistema bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ESCD0002" "12.1.10.001"}
