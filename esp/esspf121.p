...
{include/i-prgvrs.i esspf121 2.00.00.016} /*** 010016 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i esspf121 MPD}
&ENDIF
...
DEF BUFFER b-es-api-export-spf FOR es-api-export-spf.
...
FOR LAST es-api-export-spf NO-LOCK 
    WHERE  es-api-export-spf.cd-tipo-integr = 7:

    IF DATE(es-api-export-spf.data-movto) < de-ult-fech-dia
        AND es-api-export-spf.ind-situacao = 2 THEN 
       ASSIGN l-envia-medio = YES.
    ELSE
...
IF l-envia-medio THEN DO:
   ...
   /* oJsonObjMain:WriteFile("c:\temp\json-custo.json",YES,"UTF-8"). */ 
    ...
    FIND FIRST es-api-param-spf WHERE 
               es-api-param-spf.ind-tipo-trans = 2 /*---- Saida ----*/                 AND 
               es-api-param-spf.cd-tipo-integr = 7 /*---- Integra»’o atualiza‡Æo medio  ------*/ NO-LOCK NO-ERROR.
    IF AVAIL es-api-param-spf THEN 
    DO:
        IF NOT CAN-FIND(FIRST es-api-export-spf WHERE 
                              es-api-export-spf.chave        = STRING(de-ult-fech-dia,"99/99/9999") AND 
                              es-api-export-spf.ind-situacao < 2) THEN 
        DO:    
            FIND LAST b-es-api-export-spf NO-LOCK NO-ERROR.

            CREATE es-api-export-spf.
            ASSIGN es-api-export-spf.id-movto       = IF NOT AVAIL b-es-api-export-spf THEN 1 ELSE b-es-api-export-spf.id-movto + 1 //NEXT-VALUE(seq-export)
                   es-api-export-spf.chave          = STRING(de-ult-fech-dia,"99/99/9999")
                   es-api-export-spf.data-movto     = NOW
                   es-api-export-spf.cd-tipo-integr = es-api-param-spf.cd-tipo-integr     
                   es-api-export-spf.ind-situacao   = 2 /*Processado*/
                   es-api-export-spf.cod-status     = 0 /*nÆo integrado*/
                   es-api-export-spf.text-retorno   = ""
                   es-api-export-spf.c-json         = c-json.

            CREATE es-api-export-log-spf.
            ASSIGN es-api-export-log-spf.id-movto       = es-api-export-spf.id-movto
                   es-api-export-log-spf.nr-seq         = 1
                   es-api-export-log-spf.cd-tipo-integr = es-api-param-spf.cd-tipo-integr
                   es-api-export-log-spf.data-log       = NOW
                   es-api-export-log-spf.des-log        = ""      /*---- Pendente -----*/.
        ...
        RUN RealizarPost IN hComunicacaoAPI (INPUT ROWID(es-api-param-spf),
         ...
         DO:
             ...
             IF AVAIL es-api-export-spf THEN 
                ASSIGN  es-api-export-spf.data-inicio   = NOW
                        es-api-export-spf.data-fim      = NOW
                        es-api-export-spf.ind-situacao  = 2 // Processado
         ...
         FIND CURRENT es-api-export-spf EXCLUSIVE-LOCK NO-ERROR.
         IF AVAIL es-api-export-spf THEN 
            ASSIGN es-api-export-spf.clob-retorno       = cResposta
